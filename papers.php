...
<div class="container rounded-5 shadow mt-4 pt-2">
       <h1 class="text-center">Olevel Maths Past Papers</h1>
       <hr>
       <div class="row">
           <?php
            ...
            $getModelPapers = "SELECT * FROM papers WHERE p_cat=? ORDER BY p_id DESC";
            $query = $conn->prepare($getModelPapers);
            $query->execute(array($paperType));
            $row = $query->rowCount();
            if ($row > 0) {
           ...
           <img class="paper-img mt-2" src="img/playlogo.png" alt="" srcset="">
            <p><?php echo $fetch['p_name']; ?><br><span style="color: rgb(173, 167, 167);"><?php echo $fetch['p_cat']; ?></span><br><span style="color: rgb(247, 0, 0);"><?php echo $fetch['added_date']; ?></spam></p>
            <div class="justify-content-center text-center mb-2" >
                <form action="viewpdf.php" target="_blank" method="POST">
                <input type="hidden" name="link" id="" value="<?php echo $fetch['p_file']; ?>">
                 <button type="submit" name="pdflink" class="btn btn-success"><i class="fas fa-eye"></i> View Paper</button>
                </form>
